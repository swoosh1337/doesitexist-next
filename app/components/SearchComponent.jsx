...
const API_URL = 'https://doesitexist.vercel.app/api';
...
const SearchComponent = () => {
  ...
  const handleGlobeVisualization = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${API_URL}/globe-data?query=${encodeURIComponent(searchQuery)}`);
      console.log('Globe data:', response.data);
      setGlobeData(response.data);
    } catch (error) {
      console.error('Globe data error:', error);
      setError('Failed to fetch globe data: ' + error.message);
      setShowGlobe(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="bg-white shadow-md p-4">
        <h1 className="text-3xl font-bold text-center">Check if your app idea already exists</h1>
      </header>
      <main className="flex-grow p-4 overflow-auto">
        <div className="max-w-4xl mx-auto">
          ...
          {searchResults && searchResults.length > 0 && (
            <div className="mb-8">
              <h2 className="text-xl font-semibold mb-4">Top 5 Search Results</h2>
              <ul>
                {searchResults.slice(0, 5).map((result, index) => (
                  <li key={index} className="mb-4">
                    <a 
                      ...
                      className="block hover:bg-gray-100 p-2 rounded transition duration-150"
                    >
              ...
              <button
                ...
                className="mt-4 bg-purple-500 text-white py-2 px-4 rounded-md hover:bg-purple-600 transition duration-300 flex items-center justify-center"
              >
          ...
          {showGlobe && globeData && (
            <GlobeVisualization globeData={globeData.globeData} analysis={globeData.analysis} onClose={closeGlobeVisualization} />
          )}
          ...
          <div>
            <h2 className="text-xl font-semibold mb-4">Your previous searches</h2>
            <ul>
